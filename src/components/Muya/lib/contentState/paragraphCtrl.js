...
const paragraphCtrl = (ContentState) => {
  ...
  ContentState.prototype.handleFrontMatter = function () {
    ...
    if (firstBlock.type === 'pre' && firstBlock.functionType === 'frontmatter')
      return;
    ...
    if (isListed.length && !insertMode) {
      ...
      if (listType === 'order') {
        ...
        listBlock.children.forEach(
          (b) => (b.bulletMarkerOrDelimiter = orderListDelimiter)
        );
      }
      if (
        ...
        listBlock.children.forEach(
          (b) => (b.bulletMarkerOrDelimiter = bulletListMarker)
        );
      }
      ...
      if (
        ...
        const listWrapper = this.createBlock(
  ...
  ContentState.prototype.handleQuoteMenu = function (insertMode) {
    ...
    const isBlockQuote = affiliation
      .slice(0, 2)
      .filter((b) => /blockquote/.test(b.type));
      ...
      case 'paragraph': {
        ...
        if (newType !== 'p') {
          ...
          const headerContent = this.createBlock('span', {
            text:
              headingStyle === 'atx' ? newText.replace(/\n/g, ' ') : newText,
            functionType:
              headingStyle === 'atx' ? 'atxLine' : 'paragraphContent',
          });
  ...
  ContentState.prototype.insertParagraph = function (
    ...
    if (
      !anchor ||
      (anchor && anchor.functionType === 'frontmatter' && location === 'before')
    ) {
      return;
    }
  ...
  ContentState.prototype.selectAll = function () {
    ...
    if (
      ...
      if (start.key === end.key) {
        ...
        return this.muya.eventCenter.dispatch('muya-format-picker', {
          reference: null,
        });
      } else {
