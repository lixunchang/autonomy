...
const updateCtrl = (ContentState) => {
  ...
  ContentState.prototype.checkInlineUpdate = function (block) {
    ...
    if (
      /cellContent|codeContent|languageInput|footnoteInput/.test(
        block.functionType
      )
    ) {
      return false;
  ...
  ContentState.prototype.updateList = function (
    block,
    type,
    marker = '',
    line
    ...
    if (
      ...
      this.removeBlock(block);
      const isLooseListItem = preSibling.children.some(
        (c) => c.isLooseListItem
      );
      ...
      this.removeBlock(block);
      const isLooseListItem = nextSibling.children.some(
        (c) => c.isLooseListItem
      );
      nextSibling.children.forEach(
        (c) => (c.isLooseListItem = isLooseListItem)
      );
    } else {
  ...
  ContentState.prototype.updateBlockQuote = function (block, line) {
    ...
    if (/^h\d/.test(block.type)) {
      ...
      const headerContent = this.createBlock('span', {
        ...
        functionType:
          block.headingStyle === 'setext' ? 'paragraphContent' : 'atxLine',
      });
