...
class ExportHtml {
  ...
  async renderMermaid() {
    ...
    mermaid.init(
      undefined,
      this.exportContainer.querySelectorAll('div.mermaid')
    );
    if (this.muya) {
      mermaid.initialize({
  ...
  async renderDiagram() {
    const selector =
      'code.language-vega-lite, code.language-flowchart, code.language-sequence';
    const RENDER_MAP = {
  ...
  async renderHtml(toc) {
    ...
    const exportContainer = (this.exportContainer = document.createElement(
      'div'
    ));
