...
import { ensureDirSync } from './fileSystemExtra';
import { isImageFile } from './fileSystemExtra';
import { dataURItoBlob } from './fileSystemExtra';
...
export const uploadImage = async (pathname, image, preferences) => {
  ...
  const uploadByGithub = (content, filename) => {
    ...
    const message = `Upload by Mark Text at ${dayjs().format(
      'YYYY-MM-DD HH:mm:ss'
    )}`;
    var payload = {
      owner,
