...
import TurndownService, { usPluginAddRules } from './turndownService';
...
const importRegister = (ContentState) => {
    ...
    while ((token = tokens.shift())) {
      switch (token.type) {
        ...
        case 'heading': {
          ...
          value =
            headingStyle === 'atx' ? '#'.repeat(+depth) + ` ${text}` : text;
        ...
        case 'code': {
          ...
          if (/mermaid|flowchart|vega-lite|sequence/.test(lang)) {
            ...
            block = this.createBlock('pre', {
              functionType:
                codeBlockStyle === 'fenced' ? 'fencecode' : 'indentcode',
              lang,
            });
            const codeBlock = this.createBlock('code', {
              lang,
  ...
  ContentState.prototype.htmlToMarkdown = function (html, keeps = []) {
    ...
    usPluginAddRules(turndownService, keeps);
  ...
  ContentState.prototype.addCursorToMarkdown = function (markdown, cursor) {
    ...
    if (anchor.line === focus.line) {
      ...
      lines[focus.line] =
        focusText.substring(0, focus.ch) +
        CURSOR_FOCUS_DNA +
        focusText.substring(focus.ch);
    }
        ...
        if (text) {
          ...
          if (offset > -1) {
            block.text =
              ...
              text.substring(offset + CURSOR_ANCHOR_DNA.length);
            text = block.text;
            count++;
            if (editable) {
