...
export default function deleteRow(editor, cellPaths) {
  ...
  const { xRange } = getRange(...(cellRanges))
  const rangePath = getNeedHandleData(originTable, xRange)
  ...
  ;[...rangePath].reverse().forEach(([path, rowNum, needMove]) => {
    ...
    if (needMove) {
      const curCellOriginPath = originTable[path[0]][path[1]]
      const sourceOriginCol = curCellOriginPath[0][1]
