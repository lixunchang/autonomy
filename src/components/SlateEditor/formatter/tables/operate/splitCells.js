...
function getInsertRowInfo(
  ...
  const currentOriginRow = originPath[0][0]
  const range = originPath[1][0] - originPath[0][0]
  const positions = []
  let insertRow = 0
  ...
  for (let i = 1; i <= range; i++) {
    ...
    // 真实源表格行
    const currentRowIndex = Array.isArray(newOrigin[0])
      ? newOrigin[0][0]
      : newOrigin[0]
...
function dealCell(
  ...
  const { colSpan = 1, rowSpan = 1 } = cellNode[0]
...
function splitCell(editor, cellNode) {
  const { colSpan = 1, rowSpan = 1 } = cellNode[0]
  ...
  const sourceOriginCell = originTable[relativePath[0]][relativePath[1]]
  const sourceOriginCol = sourceOriginCell[0][1]
...
export default function splitCells(editor, cellPaths) {
